# Python Notebook - is_wait_prediction_v6
...
datasets['3_feat_v2_more_hist'].shape, datasets['4_feat_filter'].shape
...
df['label_delayed'] = df['wait_before_ready_time'] < -delay_thr

df['label'] = df["label_wait"].astype(str) + df["label_delayed"].astype(str)

le = LabelEncoder()
labels = le.fit_transform(df['label'])
# print(feature_arr.shape, df.shape)
...
for i in range(X_train_scaled.shape[1]):
  assert X_train_scaled[:, i].max() > 0.999, 'check if all scaled features max == 1'
...
freq = [counter[i]/len(y_test) for i in [0, 1, 2]]
print(freq)
...
le.classes_
print(le.transform(['FalseFalse', 'FalseTrue', 'TrueFalse']))
