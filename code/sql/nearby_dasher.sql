...
select 
    all_shifts.EVENT_CREATED_AT as all_shift_event_created_at,
    candidate_shifts.EVENT_CREATED_AT as candidate_event_created_at,
    SHIFT_LAT,
    SHIFT_LNG,
    case 'false' when is_idle_shift then 0 else 1 end as IS_IDLE_SHIFT,
    DASHER_ID,
    all_shifts.SP_ID,  
    all_shifts.ASSIGNMENT_RUN_ID,
    all_shifts.SHIFT_ID,
  ...
  join ingest_deepred_server_events_prod.event_deep_red_all_shifts all_shifts
    on candidate_shifts.ASSIGNMENT_RUN_ID = all_shifts.ASSIGNMENT_RUN_ID and
       candidate_shifts.SHIFT_ID = all_shifts.SHIFT_ID
  where candidate_shifts.EVENT_CREATED_AT between '2020-03-16' and '2020-03-17' and  --0415
        all_shifts.EVENT_CREATED_AT between '2020-03-16' and '2020-03-17'
);
...
from segment_events.server_events_production.deep_red_munkres_considered_delivery_stats cds
    where to_timestamp_ntz(cds.TIMESTAMP) between '2020-03-16' and '2020-04-15'
...
select 
    ...
    -- timediff(s, cds_real_time_feat.order_ready_time_utc, dd.ACTUAL_PICKUP_TIME) as late_after_ready_time
...
create table wait_geo_table as (-- get distance from one shift to one store, for each assignment_run_id
...
select 
       ...
       case when (is_nearby=1 and is_idle_shift=1) then 'nearby_idle' 
            when (is_nearby=1 and is_idle_shift=0) then 'nearby_busy' 
