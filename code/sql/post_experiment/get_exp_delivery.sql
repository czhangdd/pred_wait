...
set start_date = '2020-08-19';
set end_date = '2030-06-14';
set zone='zone1';

create or replace table chizhang.future_dasher_v8_z1_delivery_ids as(
with result as 
(
select 
assignment_run_id
...
JOIN sps on ai.sp_id=sps.sp_id
...
consider_latency as (
  ...
  inner join sps on dca.sp_id=sps.sp_id
...
solution_latency as (
  ...
  inner join sps on cds.sp_id=sps.sp_id
...
expt_group
  ...
  , route_type --added by Chi
  -- , DASHER_ASSIGNED_TIME
  -- , min_considered_time as first_tta
  , actual_delivery_time
  , ACTUAL_PICKUP_TIME
  , dasher_at_store_time
  , datediff(second, dasher_at_store_time, order_ready_time) -165 as order_ready_adj_minus_dasher_at_store
  , datediff(second, dasher_at_store_time, order_ready_time) as order_ready_minus_dasher_at_store
from staging s 
where (asap is NULL or asap < 2*60*60) and dat <2*60*60
  )